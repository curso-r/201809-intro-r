...
imdb %>% 
  ...
  ggplot() +
  geom_bar(aes(x = diretor, y = n), stat = "identity", fill = "darkblue", color = "red") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
```
...
imdb %>% 
  ...
  mutate(
    diretor = as.factor(diretor),
    diretor = fct_reorder(diretor, lucro, na.rm = TRUE)
  ) %>% 
  ggplot() +
  geom_boxplot(aes(x = diretor, y = lucro)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
